...
| **IoT Device Client — no publicly reachable pre-built artifacts** | The release CI publishes Docker images to ECR public (`aws-iot-device-client/aws-iot-device-client` and `aws-iot-device-client/aws-iot-device-client-base-images`) using an internal AWS account, but both repos return `NAME_UNKNOWN` to external callers. GitHub Releases has source archives only. You must build from source. See [docs/aws-iot-device-client-build-model.md](../../docs/aws-iot-device-client-build-model.md). |
