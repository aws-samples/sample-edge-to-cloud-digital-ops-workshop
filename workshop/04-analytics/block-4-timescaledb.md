...
This is structurally identical to **Iceberg's merge-on-read mode**: the materialization hypertable is the base data file, the raw un-refreshed chunks are the pending delete/append files, and the query-time UNION ALL is the merge-on-read planner stitching them together at scan time.
