...
**Prepared:** 2026-06-02 | **For:** Brady Joslin / June 4 planning session
**Author:** Walt Mayfield (IoT SA)

---

## Current State vs. Option Comparison

> Dimensions drawn from Brady's [workshop request](aws-iot-day2-fleet-workshop-request.pdf) (2026-05-13). **Current state** = Edge Manager + home-grown batch tool + Bash scripts.
...
## The Core Question Brady Is Asking
...
**Short answer: Yes.** IoT Jobs uses a pull/polling model over the same MQTT/mTLS connection NOV already has — no new inbound ports, no IP changes, no firewall modifications. The question is *which stack depth* makes sense given their existing `edged` + Docker Compose architecture.
...
[AWS IoT Device Client](https://github.com/awslabs/aws-iot-device-client) is a compiled C++ binary that runs as a `systemd` service *alongside* their Docker Compose stack. It handles all MQTT handshaking with AWS IoT Jobs, Device Shadows, Fleet Provisioning, and Secure Tunneling. NOV writes Day-2 logic as ordinary shell scripts (or Python) dropped into a "job handler extensions" folder — the Device Client finds and executes them when a job arrives and reports SUCCESS/FAILURE back to AWS based on exit code.

```mermaid
flowchart LR
    subgraph EDGE["Debian Edge Device"]
        subgraph DEVCLIENT["IoT Device Client — systemd service"]
            subgraph HANDLERS["job-handlers/  ← NOV writes"]
                UME["update-max-edge.sh"]
                PC["push-config.sh"]
                RC["rotate-certs.sh"]
            end
        end
        subgraph COMPOSE["Docker Compose — unchanged"]
            STACK["edged · message bus · system config"]
        end
    end

    subgraph CLOUD["AWS Cloud"]
        direction LR
        JOBS["IoT Jobs<br/>(rollout · abort · retry · scheduling)"]
        SHADOWS["Device Shadows<br/>(desired / reported state)"]
        INDEXING["Fleet Indexing<br/>Dynamic Thing Groups"]
        CATALOG["Software Package Catalog"]
    end

    EDGE -->|"MQTT/mTLS  (outbound only — no change)"| CLOUD
```
...
Instead of a separate system service, NOV adds a fourth container to their Docker Compose stack. This container owns all IoT SDK communication — subscribing to job topics, executing update logic by calling other containers (e.g., via `docker socket` or an internal API), and publishing shadow updates.

```mermaid
flowchart LR
    subgraph EDGE2["Debian Edge Device — Docker Compose Stack"]
    ...
    subgraph CLOUD2["AWS Cloud (same as Option A)"]
        direction LR
        JOBS2["IoT Jobs"]
...
- The team already chose Docker Compose *deliberately* because Greengrass components were "problematic" for their Go-Land components (per prior Nick Zuchlewski conversation)
...
### Option A — IoT Jobs + Device Client: Limited public evidence, strong internal signal
...
- **Internal AWS positioning note:** Device Client design docs describe the target customer as "those interested in creating PoCs who do not yet need or want to pay for the sophisticated management of Greengrass V2." This is the *initial* target — the intended path is PoC → production. It does not mean Device Client is unsuitable for production; it means AWS hasn't yet built the case study library for it.

**Bottom line for NOV:** Device Client is the right starting point for the workshop, and the pattern is sound for production at NOV's scale. The lack of a published case study is a marketing gap, not a technical gap. Reach out to `#aws-iot-wwso-all` for NDA-protected references if needed before committing to production.
...
Rather than building a custom workshop from scratch, Jeff and Walt can base the NOV session on existing AWS Workshop Studio content — customizing the job handler scenarios to match NOV's actual update operations.
...
**Delivery note:** IoT Immersion Days are delivered by the account SA — that's you, Walt. For basic IoT Immersion Days, account SAs deliver without needing an IoT Specialist SA present. Issues with workshop content → file a ticket at `C/T/I: AWS Solutions Architecture / Specialist / IoT Workshops`.
...
- **Intermittent WAN connectivity:** Rigs and frac vans on satellite or LTE links experience frequent disconnects. A durable edge-side log (e.g., [Redpanda](https://redpanda.com/) or [Apache Kafka](https://kafka.apache.org/) at the edge) can buffer and replay from the last committed offset when connectivity restores — no data loss, no application-level retry logic required.
...
- Choosing between Device Client, SDKs, Greengrass, FreeRTOS — see internal field guidance; public summary: prefer Device Client for Jobs/Shadows/Tunneling on Linux without GG dependency
